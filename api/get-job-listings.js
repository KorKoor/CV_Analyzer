...
function extractTag(html, tag, cls) {
  const p = cls
    ? new RegExp(`<${tag}[^>]*class="[^"]*${cls}[^"]*"[^>]*>(.*?)<\\/${tag}>`, "is")
    : new RegExp(`<${tag}[^>]*>(.*?)<\\/${tag}>`, "is");
  return html?.match(p)?.[1]?.replace(/<[^>]+>/g, "").trim() || null;
}
function extractAttr(html, tag, attr) {
  return html?.match(new RegExp(`<${tag}[^>]*\\s${attr}="([^"]*)"`, "i"))?.[1] || null;
}
function extractRawTag(html, tag) {
  return html?.match(new RegExp(`<${tag}[^>]*>([\\s\\S]*?)<\\/${tag}>`, "i"))?.[1] || null;
}
...
function extractSalary(text = "") {
  return (
    text.match(/\$[\d,]+\s*(?:–|-|a)\s*\$[\d,]+(?:\s*(?:MXN|pesos|mensual))?/i)?.[0] ||
...
function syntheticJob(title, location, source, url) {
  return { title, company: null, location, url,
...
function parseLinkedin(html, title, location) {
  if (!html) return [syntheticJob(title, location, "LinkedIn", buildSearchUrl("LinkedIn", title, location))];
  const jobs = [];
  ...
  return jobs.length > 0
    ? jobs
    : [syntheticJob(title, location, "LinkedIn", buildSearchUrl("LinkedIn", title, location))];
}

function parseIndeedRSS(xml, title, location) {
  if (!xml) return [syntheticJob(title, location, "Indeed", buildSearchUrl("Indeed", title, location))];
  const jobs = [];
  ...
  return jobs.length > 0
    ? jobs
    : [syntheticJob(title, location, "Indeed", buildSearchUrl("Indeed", title, location))];
}

function parseOCC(html, title, location) {
  if (!html) return [syntheticJob(title, location, "OCC Mundial", buildSearchUrl("OCC Mundial", title, location))];
  const jobs = [];

  for (const [, block] of html.matchAll(/<article[^>]*>([\s\S]*?)<\/article>/gi)) {
    const jobTitle = extractTag(block, "h2", "") || extractTag(block, "h3", "");
    const company  = extractTag(block, "span", "company") || extractTag(block, "a", "company");
    const loc      = extractTag(block, "span", "location") || extractTag(block, "span", "city");
    const rawLink  = extractAttr(block, "a", "href");
    ...
    if (jobTitle) {
      jobs.push({
        title: cleanText(jobTitle), company: cleanText(company),
        location: cleanText(loc) || location,
        url: fullUrl, postedAt: null,
        modality: detectModality(block),
        salary: extractSalary(block),
        description,
        source: "OCC Mundial", synthetic: !rawLink,
      });
    }
  }
  ...
  return jobs.length > 0
    ? jobs
    : [syntheticJob(title, location, "OCC Mundial", buildSearchUrl("OCC Mundial", title, location))];
}
...
function parseComputrabajo(html, title, location) {
  ...
  for (const [, block] of html.matchAll(/<article[^>]*>([\s\S]*?)<\/article>/gi)) {
    ...
    if (jobTitle) {
      jobs.push({
        ...
        salary: extractSalary(block),
        description,
        source: "Computrabajo", synthetic: !rawLink,
      });
    }
  }
  ...
  return jobs.length > 0
    ? jobs
    : [syntheticJob(title, location, "Computrabajo", buildSearchUrl("Computrabajo", title, location))];
}

function parseGetOnBrd(json, title, location) {
  if (!json) return [syntheticJob(title, location, "GetOnBrd", buildSearchUrl("GetOnBrd", title, location))];
  let data;
  try { data = JSON.parse(json); } catch {
    return [syntheticJob(title, location, "GetOnBrd", buildSearchUrl("GetOnBrd", title, location))];
  }
  const items = Array.isArray(data) ? data : (data.data || data.jobs || []);
  if (!items.length) return [syntheticJob(title, location, "GetOnBrd", buildSearchUrl("GetOnBrd", title, location))];
...
async function scrapePortal(portal, title, location, maxJobs) {
  try {
    ...
    return { portal: portal.name, searchUrl: buildSearchUrl(portal.name, title, location), jobs: jobs.slice(0, maxJobs) };
  } catch {
    return { portal: portal.name, searchUrl: buildSearchUrl(portal.name, title, location), jobs: [syntheticJob(title, location, portal.name, buildSearchUrl(portal.name, title, location))] };
  }
}
...
export async function getJobListings(req, res) {
  ...
  if (!title?.trim()) return res.status(400).json({ error: "Falta el parámetro 'title'" });

  res.setHeader("Cache-Control", "s-maxage=900, stale-while-revalidate=1800");

  const keys    = portals.split(",").map(p => p.trim().toLowerCase()).filter(k => PORTALS[k]);
  const maxJobs = Math.min(Math.max(parseInt(limit) || 2, 1), 3);

  const settled = await Promise.allSettled(
    keys.map(k => scrapePortal(PORTALS[k], title.trim(), location.trim(), maxJobs))
  );
  ...
  const modalities = [...new Set(allJobs.map(j => j.modality).filter(Boolean))];
